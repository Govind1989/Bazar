...
export default function ProductListView({ title, description, type, icon }: ProductListViewProps) {
  ...
  const FiltersContent = () => (
    <div className="space-y-10">
       <div>
          <Typography variant="titleSm" className="uppercase tracking-[0.2em] text-[10px] opacity-40 mb-6">Categories</Typography>
      ...
      <section className="px-6 md:px-12 py-12 border-b border-bazar-gray-100 dark:border-bazar-gray-900 bg-bazar-gray-50/50 dark:bg-bazar-gray-950/50">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 mb-6 text-xs font-mono uppercase tracking-widest opacity-40">
             ...
             <span className="text-bazar-black dark:text-bazar-white">{title}</span>
          </div>
          <div className="flex items-center gap-4 mb-4">
             {icon && <div className="w-12 h-12 rounded-full bg-bazar-black dark:bg-bazar-white text-bazar-white dark:text-bazar-black flex items-center justify-center shadow-lg">{icon}</div>}
             <Typography variant="displayLg">{title}</Typography>
          </div>
          <Typography variant="bodyMd" className="max-w-xl opacity-60">
            {description} Discover {products?.length || 0} exclusive offers currently active.
          </Typography>
        </div>
      </section>
